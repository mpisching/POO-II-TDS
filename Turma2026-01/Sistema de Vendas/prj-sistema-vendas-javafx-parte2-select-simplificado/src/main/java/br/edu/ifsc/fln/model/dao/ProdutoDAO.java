...
//                + "FROM produto p INNER JOIN categoria c ON c.id = p.id_categoria;";
    ...
    public List<Produto> listarPorCategoria(Categoria categoria) {
        String sql = "SELECT * FROM PRODUTO WHERE WHERE id_categoria = ?;";
        List<Produto> retorno = new ArrayList<>();
        try {
            PreparedStatement stmt = connection.prepareStatement(sql);
            ...
            ResultSet resultado = stmt.executeQuery();
            while (resultado.next()) {
                Produto produto = populateVO(resultado, true);
     ...
     * @throws SQLException 
     */
    private Produto populateVO(ResultSet rs, boolean comCategoria) throws SQLException {
