...
import br.edu.ifsc.fln.service.CupomFiscalService;
...
public class FXMLAnchorPaneProcessoVendaController implements Initializable {
    ...
    private void handleButtonInserir(ActionEvent event) throws IOException, SQLException {
        ...
        if (buttonConfirmarClicked) {
            vendaDAO.setConnection(connection);
            int ultimoId = vendaDAO.inserir(venda); //retorna o último id de venda para impressão do cupom
            if (venda.getStatusVenda() == EStatusVenda.FINALIZADA) {
                CupomFiscalService.imprimirCupomFiscal(ultimoId);
            }
            carregarTableView();
        }
    }
    ...
    private void handleButtonAlterar(ActionEvent event) throws IOException {
        ...
        if (venda != null) {
            ...
            if (buttonConfirmarClicked) {
                ...
                if (venda.getStatusVenda() == EStatusVenda.FINALIZADA) {
                    CupomFiscalService.imprimirCupomFiscal(venda.getId());
                }
                carregarTableView();
            }
        } else {
